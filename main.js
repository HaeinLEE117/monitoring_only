...
//1층이 아닌 장소에 있는 agv는 2, 3 층 버튼 밑에 위치하도록 
      if ((parseInt(f1_data.position[i]) / 1000) > 2) {
        f1_data.ScaleX[i] =(420);
        f1_data.ScaleY[i] = (68 +  i * 30);
      } else if ((parseInt(f1_data.position[i]) / 1000) > 1) {
        f1_data.ScaleX[i]=(247);
        f1_data.ScaleY[i]=(88 +  i * 30);
      }
      if (f1_data.vehicle_running[i] === "#02c706") {
        f1_data.line_color[return_line(f1_data.destination[i])] = f1_data.color[i];
      }
    }
